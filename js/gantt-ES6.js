$(document).ready(function () {
  ...
  const renderTable = (titleData, lineData) => {
    ...
    if (titleData.value === 'quarter' || titleData.value === 'year') {
      titleData.content.map((item, index) => {
        let rowWidth = 0
        // 如果是季度，年的首个表格，则宽度再格子数*70的基础上再+1
        if (index === 0) {
  ...
  const renderProgress = (progressData, calibration) => {
    ...
    const renderDateProgress = (progressVal, progressDate) => {
      progressVal.map((item, index) => {
        ...
        let endTime = item.lineContent.end.split('/')
        // new Date()第3个参数默认为1，就是每个月的1号，把它设置为0时， new Date()会返回本月的最后一天，然后通过getDate()方法得到天数
        let curStartDate = new Date(startTime[0], startTime[1] - 0, 0).getDate()
        // 开始任务的日期至当月结束日期的时间段
        let remainStartDate = curStartDate + 1 - startTime[2]
        // 中间时间段（跨月/年）
        let plusDate = 0
        // 总时间跨度(占据的格子数)
        let timeNum = 0
        ...
        if (startTime[0] === endTime[0]) {
          ...
          if (startTime[1] === endTime[1]) {
            timeNum = parseInt(endTime[2]) + 1 - startTime[2]
            // 跨月
          } else {
        ...
        const leftStartDate = progressDate.content[0].subTitle[0].dateNum
        // 起始日到起始日所在月份月底的时间段
        const leftRemainDate = new Date(leftStartYear, leftStartMonth, 0).getDate() - leftStartDate + 1
        // console.log('起始日到起始日所在月份月底的时间段',leftRemainDate)
        // 任务条左偏移的格子数
        let progressLeft = 0
        // 任务条左偏移的时间段天数
        let leftPlusDate = 0
        // 当前任务起始时间与最早任务的起始时间在同一年
        if (startTime[0] === leftStartYear) {
          // 当前任务起始时间与最早任务的起始时间在同一月
          if (startTime[1] === leftStartMonth) {
            // 左偏移从结尾时间点开始，所以不需要+1
            progressLeft = startTime[2] - leftStartDate
            // 当前任务起始时间与最早任务的起始时间不在同一月
          } else {
    ...
    const renderMonthProgress = (progressVal, progressMonth) => {
      ...
      progressVal.map((item, index) => {
        ...
        let remainStartMonth = 12 - startTime[1] + 1
        // 结束任务所在年份的1月至结束任务的月所包含的月份数
        let remainEndMonth = endTime[1] - 0
        // 中间时间段（跨月/年）
        let plusMonth = 0
        // 总时间跨度(占据的格子数)
        let timeNum = 0
        if (startTime[0] === endTime[0]) {
          timeNum = endTime[1] - startTime[1] + 1
          // 时间跨度跨年
        } else {
          // 中间相差的年数包含的月份总和
          for (let i = endTime[0] - 1; i > startTime[0]; i--) {
        ...
        const leftStartMonth = parseInt(progressMonth.content[0].subTitle[0])
        // 任务条左偏移的格子数
        let progressLeft = 0
        // 任务条左偏移的时间段月份数
        let leftPlusMonth = 0
        // 起始月到起始月所在年末的月份总和
        const leftRemainMonth = 12 - leftStartMonth
        // 当前任务起始时间与最早任务的起始时间在同一年
        if (startTime[0] === leftStartYear) {
          progressLeft = startTime[1] - leftStartMonth
          // 当前任务起始时间与最早任务的起始时间不在同一年
        } else {
          // 中间相差的年数包含的月份总和
    ...
    const renderQuarterProgress = (progressVal, progressQuarter) => {
      progressVal.map((item, index) => {
        ...
        let endTime = item.lineContent.end.split('/')
        // 开始任务的月份至当年12月所包含季度数
        let remainStartQuarter = parseInt((12 - startTime[1]) / 3 + 1)
        // 结束任务所在年份的1月至结束任务的月所包含的季度数
        let remainEndQuarter = parseInt((endTime[1] - 1) / 3 + 1)
        // 中间时间段（跨季度/年）
        let plusQuarter = 0
        // 总时间跨度(占据的格子数)
        let timeNum = 0
        ...
        if (startTime[0] === endTime[0]) {
          ...
          timeNum = parseInt((endTime[1] - startTime[1]) / 3 + 1)
          // 时间跨度跨年
        } else {
          // 中间相差的年数包含的季度数总和
          for (let i = endTime[0] - 1; i > startTime[0]; i--) {
        ...
        const leftStartWord = progressQuarter.content[0].subTitle[0]
        // 将汉字季度转义为数字
        ...
        let progressLeft = 0
        // 任务条左偏移的季度数
        let leftPlusQuarter = 0
        // 最早起始季到所在年末的季度总和
        const leftRemainQuarter = 5 - leftStartQuarter
        // 当前任务的起始季度
        const curStartQuarter = parseInt((startTime[1] - 1) / 3 + 1)
        // 当前任务起始时间与最早任务的起始时间在同一年
        if (startTime[0] === leftStartYear) {
          progressLeft = curStartQuarter - leftStartQuarter
          // 当前任务起始时间与最早任务的起始时间不在同一年
        } else {
          // 中间相差的年数包含的季度数总和
          for (let i = startTime[0] - 1; i > leftStartYear; i--) {
    ...
    const renderYearProgress = (progressVal, progressYear) => {
      progressVal.map((item, index) => {
        ...
        // console.log(startTime, endTime)
        // 开始任务的月份至当年12月所包含半年数
        let remainStartYear = parseInt((12 - startTime[1]) / 6 + 1)
        // 结束任务所在年份的1月至结束任务的月所包含的半年
        let remainEndYear = parseInt((endTime[1] - 1) / 6 + 1)
        // 中间时间段（跨季度/年）
        let plusYear = 0
        // 总时间跨度(占据的格子数)
        let timeNum = 0
        ...
        if (startTime[0] === endTime[0]) {
          ...
          timeNum = parseInt((endTime[1] - startTime[1]) / 6 + 1)
          // 时间跨度跨年
        } else {
          // 中间相差的年数包含的年数总和
          for (let i = endTime[0] - 1; i > startTime[0]; i--) {
        ...
        const leftStartWord = progressYear.content[0].subTitle[0]
        // 将汉字上下半年转义为数字
        let leftNumYear = 0
        switch (leftStartWord[0]) {
          case '上':
        ...
        // 任务条左偏移的季度数
        let leftPlusYear = 0
        // 最早时间起始的半年到所在年末的半年数总和
        const leftRemainYear = 3 - leftNumYear
        // 当前任务的起始半年数
        const curStartYear = parseInt((startTime[1] - 1) / 6 + 1)
        // 当前任务起始时间与最早任务的起始时间在同一年
        if (startTime[0] === leftStartYear) {
          progressLeft = curStartYear - leftNumYear
          // 当前任务起始时间与最早任务的起始时间不在同一年
        } else {
          // 中间相差的年数包含的半年数总和
          for (let i = startTime[0] - 1; i > leftStartYear; i--) {
  ...
  const calculateWidth = () => {
    ...
    var tableItemWidth = 0
    // console.log(tableItems)
    tableItems.each(function () {
      tableItemWidth += $(this).outerWidth()
      // console.log('获取表格DOM宽',$(this).outerWidth())
  ...
  const mousePosition = () => {
    $('.model-gantt-table-mask-progress').on('mousemove', function (e) {
      ...
      // 在表格里滑动时再显示定位条
      $('.model-gantt-table-current').addClass('active')
      // 鼠标获取X轴上的格子数
      let xNum = parseInt(e.offsetX / 35)
      // 跨列的格子数
      let xNumMax = parseInt(e.offsetX / 70)
      // 鼠标获取Y轴上的格子数
      let yNum = parseInt(e.offsetY / 35) + 1
      // 当前定位的X，Y轴变蓝色
      let positionX = xNum * 35
      // 跨列的X定位
      let positionXMax = xNumMax * 70
      let positionY = yNum * 35 + 36
      // 定位到父元素取margin-left
      let parentLeft = parseInt($(e.target).parents(".model-gantt-table-mask-progress-item").css('margin-left'))
      // 直接取到本元素的margin-left
      let itemLeft = parseInt($(e.target).css('margin-left'))
      // 日期标识
  ...
  const alertMsg = () => {
    // 任务数据的最大值
    let maxIndex = missionData.missionContent.length
    // 当前被点击任务的索引
    let msgIndex = 0
    $('.model-gantt-table-mask-progress-item').on('click', function (e) {
      // console.log(e.clientY)
      // console.log('===', e.offsetX)
      msgIndex = $(this).attr('index')
      // 提示窗标题
      let msgTitle = missionData.missionContent[msgIndex].lineContent.desc
      // 开始时间
      let startTime = missionData.missionContent[msgIndex].lineContent.start
      // 结束时间
      let endTime = missionData.missionContent[msgIndex].lineContent.end
      // 进度，toFixed解决浮点计算精度丢失导致的计算异常问题
      let progressPercent = (missionData.missionContent[msgIndex].lineContent.progress * 100).toFixed(2)
      // 进度条左偏移数值
      ...
      $('.model-gantt-table-msg-content-percent span').html(`${progressPercent}%`)
      // 显示提示窗，修改显示位置
      $('.model-gantt-table-msg').addClass('active')
      // 保证数据少的时候，弹出面板能正常显示，将定位改为fixed定位后，需要判断抽屉层是否展开，决定左偏移的值
  ...
  const clickFilter = () => {
    $('.model-gantt-drawer-setting .model-gantt-checkbox-desc').on('click', function () {
      // 当前被点击的DOM索引
      let curIndex = $(this).parents('.model-gantt-drawer-setting-item').index()
      // 选中标识
      let checkedFlag = $(this).siblings().attr('class').indexOf('checked')
      // 与被点击DON索引相同的抽屉头部列
      let tableHeader = $(this).parents('.model-gantt-drawer').find('.model-gantt-drawer-wrap thead tr td').eq(curIndex + 2)
      // 与被点击DON索引相同的抽屉内容行
      let tableLine = $(this).parents('.model-gantt-drawer').find('.model-gantt-drawer-wrap tbody tr')
      // console.log($(tableLine[0]).children().eq(curIndex + 2))
      // 如果为选中状态
      if (checkedFlag !== -1) {
        tableHeader.css('display', 'table-cell')
        // 显示表格列
    ...
    $('.model-gantt-drawer-setting  .model-gantt-checkbox-wrap input').on('click', function () {
      ...
      let curIndex = $(this).parents('.model-gantt-drawer-setting-item').index()
      // 选中标识
      let checkedFlag = $(this).parent().attr('class').indexOf('checked')
      // 与被点击DON索引相同的抽屉头部列
      let tableHeader = $(this).parents('.model-gantt-drawer').find('.model-gantt-drawer-wrap thead tr td').eq(curIndex + 2)
      // 与被点击DON索引相同的抽屉内容行
      let tableLine = $(this).parents('.model-gantt-drawer').find('.model-gantt-drawer-wrap tbody tr')
  ...
  const deleteRow = () => {
    let deleteArr = []
    let selectAll = $('.select-all').children()
    // 全选逻辑
    $('.select-all').on('click', function () {
      // 全选时ID数组先置为空
      deleteArr = []
      // 多选项
      let selectBox = $(this).parents('thead').siblings().find('.model-gantt-checkbox-wrap')
      // 多选项ID
      let checkedArr = $(this).parents('thead').siblings().children()
      // 选中标示
      let checkedFlag = $(this).children().attr('class').indexOf('checked')
      // 选中状态
      if (checkedFlag !== -1) {
    ...
    $('.model-gantt-drawer-wrap tbody tr').on('click', '.model-gantt-checkbox-wrap input', function (e) {
      ...
      // 被选中行的ID值
      let checkedId = $(this).parents('tr').attr('fdid')
      // 选中标识
      let checkedFlag = $(this).parents('.model-gantt-checkbox').children('.model-gantt-checkbox-wrap').attr('class').indexOf('checked')
      // 被点击的单选项的兄弟元素
      let checkSiblings = $(this).parents('tr').siblings().find('.model-gantt-checkbox-wrap')
      // 选中状态把对应id推入deleteArr中，当全部按钮被选中时，全选按钮高亮
      if (checkedFlag !== -1) {
  ...
  const renderInit = () => {
    ...
    renderPositionElement(timeData.dataArr[3])
    // 功能型函数
    calculateWidth()
    operateDrawer()
    mousePosition()
